...
echo "[4/6] uploading v2.1 files..."
for f in paper/cls-ledger-preprint-v2.1.pdf paper/draft.md \
...
md['description'] = md['description'].replace(
    '</p>',
    ' <b>v2.1 (no results changed):</b> Related Work expanded with '
    'concurrent 2025-2026 systems after a four-agent adversarial '
    'prior-art search - deterministic staleness control for retrieval '
    '(MemStrata, TOKI, STALE/CUPMem, deterministic freshness selection), '
    'lifelong model editing with isolated modules (GRACE, MELO, WISE, '
    'K-Adapter), and adapter-based unlearning (FUSED, S3T) - sharpening '
    'the contribution claim to the coupling: a provenance ledger '
    'governing entity-level weight slots.</p>', 1)
md = {k: v for k, v in md.items() if v is not None}
json.dump({'metadata': md}, open('/tmp/zen_md.json', 'w'))
print('  metadata prepared (version=v2.1, description extended; '
...
echo "[6/6] publishing (mints v2.1 DOI — irreversible)..."
...
echo "PUBLISHED v2.1"
echo "  v2.1 DOI    : $DOI"
echo "  concept DOI : 10.5281/zenodo.21375428 (now resolves to v2.1)"
